...
DEPS = {"binutils": "2.46.0", "gcc": "15.2.0"}
...
HOST_ENVIRONMENT.Append(PROJECTDIR=HOST_ENVIRONMENT.Dir(".").srcnode())
...
toolchainDir = Path(
    HOST_ENVIRONMENT["toolchain"], RemoveSuffix(platform_prefix, "-")
).resolve()
toolchainBin = Path(toolchainDir, "bin")
toolchainGccLibs = Path(
    toolchainDir, "lib", "gcc", RemoveSuffix(platform_prefix, "-"), DEPS["gcc"]
)
...
SConscript(
    "src/libs/core/SConscript", variant_dir=variantDir + "/libs/core", duplicate=0
)
